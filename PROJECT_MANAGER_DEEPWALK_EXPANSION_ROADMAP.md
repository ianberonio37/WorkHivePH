...
| **G4** | **One permissive `ALL` policy per table = any active member can approve a change order.** | `approveCO` / `rejectCO` / `cancelCO` are money decisions. `project_roles` is the ONLY table with a supervisor predicate; the other six are open to every member of the hive. |
