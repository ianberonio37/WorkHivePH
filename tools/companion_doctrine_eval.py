"""
Companion Doctrine Eval Harness — Probe Taxonomy family H (folds into safety/persona).
=====================================================================================
Drives the generic marker grader over companion_doctrine_golden.json. Doctrine = does the companion
honour WorkHive doctrine (maturity-gate honesty, never-replaces-ERP, floating-companion grounding,
free-platform, low-infra empathy)? markers_all = the honest/guardrail phrasing; anti_markers = the
doctrine violation. Deterministic, independent. Each unit carries its target dimension (safety|persona).
Same template as companion_robustness_eval.py.
...
GOLDEN_PATH = ROOT / "companion_doctrine_golden.json"
DIMENSION = "doctrine"
...
def grade_observed(observed_path: Path, out_path: Path) -> int:
    ...
    for unit in (golden.get("probes") or golden.get("units") or []):
        ...
        results.append({"id": uid, "passed": is_pass, "score": 100 if is_pass else 0,
                        "probe_type": unit.get("probe_type"), "dimension": unit.get("dimension"),
...
def main() -> int:
    ap = argparse.ArgumentParser(description=f"{DIMENSION} eval harness (Probe Taxonomy family H)")
