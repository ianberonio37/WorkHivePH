...
BY_DESIGN = {
    ...
    "marketplace_inquiries": "TWO-PARTY marketplace transaction table (the buyer↔seller message on a listing) — cross-hive by design, exactly like marketplace_listings/sellers: a buyer in hive A inquiring about a hive-B seller's listing produces a hive_id=B row BOTH parties must read. mkt_inq_read is party-scoped (buyer_name OR seller_name IN auth_worker_names() OR is_marketplace_admin()); the gate's hive_id-isolation model does not fit a party-scoped table, and its 'leak' came from picking ADMIN members (admin-see-all is by-design) — VERIFIED 2026-09-01 RLS-enforced as a NON-ADMIN worker: total visible 2, non-party 0 (sees ONLY rows where they are buyer/seller). Residual: scoping is by worker_name, so a name collision across hives could over-share — the SAME Arc-R auth_uid-resolution backlog flagged on marketplace_sellers, not a new hole.",
    "marketplace_orders": "TWO-PARTY marketplace transaction table (the order between a buyer and a hive-B seller) — cross-hive by design, sibling of marketplace_inquiries. mkt_orders_read is party-scoped (buyer/seller name OR admin); the grafana read policy is TO grafana_reader (a service role), NOT authenticated, so it does not widen a normal user. VERIFIED 2026-09-01 RLS-enforced as a NON-ADMIN worker: total visible 2, non-party 0. Same name-scoping Arc-R backlog as above.",
    "platform_feedback": "global public product-feedback board",
    "service_providers": "service-hailing PROVIDER DIRECTORY (SERVICE_HAILING_ROADMAP.md §1, the service sibling of marketplace_sellers) — hailing requires clients to discover providers across hives (display_name/categories/service_areas/base_lat-lng/availability/verified = provider-public by design). Verified 2026-07-28 (P1 adversarial suite T9/T10): the authenticated column GRANT excludes live_location AND auth_uid entirely (revoke-first, mig 20260728000024) — live tracking is only reachable via the P2 active-job DEFINER view, and unlike marketplace_sellers there is NO client-visible auth_uid to gate later.",
}
