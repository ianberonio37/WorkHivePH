...
SWEEP_CRITIQUES_PATH = ROOT / "sweep_critiques.json"          # Grounded MCP Sweep harsh-critic feed (exploratory candidate class)
...
def _sweep_critique_candidates(dispositions: dict) -> list:
    """Grounded MCP Sweep harsh-critic recs (sweep_critiques.json) as an
    exploratory candidate class. The MCP sweep is the EXPLORATORY layer of the
    self-improving gate (driving live pages); its 'should-be' recs feed this same
    promotion engine so a sweep finding is dispositioned exactly like a miner
    pattern. Unlike miner patterns these are deliberate human findings, so they
    are NOT recurrence-gated — valid the first turn raised — but they honour the
    same disposition mechanism (drop once approved/rejected/snoozed)."""
    doc = _load_json_path(SWEEP_CRITIQUES_PATH)
    out: list = []
    if not doc:
        return out
    sev_rank = {"Blocker": 0, "Major": 1, "Minor-Major": 1, "Minor": 2, "Polish": 3}
    for c in doc.get("critiques", []) or []:
        key = c.get("key")
        if not key:
            continue
        if _disposition_status(dispositions, key) in ("approved", "rejected", "snoozed"):
            continue
        out.append(c)
    out.sort(key=lambda c: (sev_rank.get(c.get("severity", "Minor"), 2), c.get("key", "")))
    return out
...
def _compute_promotions(curr: dict, state: dict, turn: int) -> dict:
    ...
    # Grounded-sweep critiques — exploratory candidate class, not recurrence-gated.
    sweep_q = _sweep_critique_candidates(dispositions)

    return {"rule_candidates": rule_q, "sentinel_candidates": sent_q,
            "sweep_candidates": sweep_q,
...
def _write_promotion_queue(promo: dict, turn: int, ts: str) -> None:
    ...
    swp = promo.get("sweep_candidates", [])
    L: list = []
    ...
    L.append(f"**Summary:** {len(rc)} rule candidate(s) · {len(sc)} sentinel candidate(s) · "
             f"{len(swp)} grounded-sweep critique(s) · "
    ...
    L.append("---\n")
    L.append("## Exploratory → L2/L0  ·  Grounded MCP Sweep critiques (live-page harsh-critic)\n")
    if swp:
        L.append("The MCP sweep drives each page LIVE and proposes opinionated 'should-be' recs "
                 "(`sweep_critiques.json`, mirror: `SWEEP_CRITIQUE_QUEUE.md`). Not recurrence-gated — "
                 "a deliberate finding is valid the first time raised. Promote = fix + crystallize a "
                 "lock (a `journey-*.spec.ts` assertion, tracked by `validate_grounded_sweep.py`) or "
                 "author an L0 validator.\n")
        for i, c in enumerate(swp, 1):
            L.append(f"### {i}. {c.get('title','(untitled)')}  ({c.get('page','?')})")
            L.append(f"- **key:** `{c.get('key','')}`")
            L.append(f"- **signal:** {c.get('flag','?')} · pillar {c.get('pillar','?')} · "
                     f"sev {c.get('severity','?')} · effort {c.get('effort','?')} · wave {c.get('wave','?')}")
            if c.get("should_be"):
                L.append(f"- **should be:** {c['should_be']}")
            L.append("- [ ] approve → fix + crystallize a journey lock (or `/harden` an L0 validator)\n")
    else:
        L.append("_No open grounded-sweep critiques (all dispositioned, or sweep_critiques.json absent)._\n")
