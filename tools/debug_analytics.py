#!/usr/bin/env python3
"""Debug analytics.html - check network requests and final verdict state"""
...
with sync_playwright() as p:
    ...
    page.on("response", lambda res: requests_log.append(f"RES: {res.status} {res.url}"))
    ...
    print("Loading analytics.html...")
    page.goto(BASE_URL + "/analytics.html", wait_until="domcontentloaded")
    print("Waiting 12s for AI verdict...")
    page.wait_for_timeout(12000)

    # Check verdict element
    verdict_label = page.text_content("#an-verdict-label") or ""
    verdict_sub = page.text_content("#an-verdict-sub") or ""
    print(f"\nVerdict label: '{verdict_label}'")
    print(f"Verdict sub: '{verdict_sub[:200]}'")

    print("\n=== NETWORK REQUESTS (analytics-related) ===")
    for log in requests_log:
        if "analytic" in log.lower() or "supabase" in log.lower() or ":8000" in log:
            print(f"  {log[:200]}")
