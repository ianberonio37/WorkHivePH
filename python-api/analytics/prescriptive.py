...
def calc_priority_ranking(
    ...
    # Build criticality map from pm_assets
    crit_map: dict = {}
    if not assets.empty and "asset_name" in assets.columns and "criticality" in assets.columns:
        for _, row in assets.iterrows():
            name = str(row.get("asset_name", "") or "").strip()
            crit = str(row.get("criticality", "") or "").strip()
            if name:
                crit_map[name.lower()] = crit
    ...
    for _, row in failure_stats.iterrows():
        ...
        # Match criticality by asset name
        crit_key    = machine.lower()
        ...
        ranking.append({
            ...
            "priority":     "P1" if risk_score >= 20 else "P2" if risk_score >= 8 else "P3",
        })
...
def calc_pm_interval_optimization(
    ...
    for machine, group in log.groupby("machine"):
        ...
        if len(dates) >= 2:
            ...
            mtbf_map[machine.lower()] = float(intervals.mean())

    # Build asset_name → criticality map
    crit_map: dict = {}
    assets_df = _to_df(pm_assets)
    if not assets_df.empty and "asset_name" in assets_df.columns:
    ...
    if "asset_name" not in scope.columns:
        return {"recommendations": [], "note": "asset_name not available in scope items",
                "standard": "SAE JA1011 §7 RCM interval optimisation"}

    for asset_name, s_group in scope.groupby("asset_name"):
        asset_lower = asset_name.lower()
        mtbf = mtbf_map.get(asset_lower)
        if mtbf is None:
            continue  # no failure history to compare against

        crit = crit_map.get(asset_lower, "Medium")

        for _, item in s_group.iterrows():
            freq     = item.get("frequency", "Monthly")
            freq_day = FREQ_DAYS.get(freq, 30)

            if mtbf < freq_day * 0.8:
                # MTBF significantly shorter than PM interval → increase frequency
                action      = "INCREASE FREQUENCY"
                recommended = max(7, int(mtbf * 0.5))
                reason      = f"MTBF ({round(mtbf,0)}d) is shorter than current PM interval ({freq_day}d). Failures are occurring between PMs."
            elif mtbf > freq_day * 5 and crit not in ("Critical", "High"):
                # MTBF much longer → may be over-maintained
                action      = "REVIEW — MAY REDUCE"
                recommended = int(freq_day * 2)
                reason      = f"MTBF ({round(mtbf,0)}d) is much longer than current interval ({freq_day}d). Consider extending for non-critical assets."
            else:
                continue  # current interval is appropriate

            recommendations.append({
                "asset_name":         asset_name,
                "task":               str(item.get("item_text", ""))[:60],
                "current_frequency":  freq,
                "current_interval_d": freq_day,
...
def calc_technician_assignment(
    ...
    if not badges.empty and "discipline" in badges.columns:
        ...
        for disc, group in top.groupby("discipline"):
            best_row = group.loc[group["level"].idxmax()]
            best_by_disc[disc] = {
                "worker_name": best_row["worker_name"],
    ...
    CAT_TO_DISC = {
        ...
        "Instrumentation": "Instrumentation", "Civil": "Civil",
        "HVAC": "Mechanical", "Hydraulics": "Mechanical",
    ...
    for _, job in log.head(10).iterrows():
        ...
        if best:
            assignments.append({
                ...
                "reason":          f"Highest Level {best['level']} {disc} tech available.",
            })
        else:
            skill_gaps.append({
                "machine":     machine,
