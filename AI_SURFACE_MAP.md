...
**NEXT: 8.3** — run the Agent golden transcripts through the live gateway (`companion_agent_eval --observed`), freeze the Agent baseline on the 10 locked-test units, and register the per-dimension G0 regression gate in `run_platform_checks.py`. Then replicate 8.1+8.2 for **RAG** (Ragas-4 + ground-truth chunk ids).
