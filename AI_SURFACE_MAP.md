...
**NEXT: replicate the Agent template for RAG** (8.1 golden = question + ground-truth answer + ground-truth chunk ids; 8.2 grader = Ragas-4 with `cited[]` deterministic context precision/recall + judge-with-backstop faithfulness/relevancy; 8.3 freeze + gate). Then Memory (LongMemEval 5 abilities + abstention) and Persona (voice markers). **Follow-up for Agent:** expand the golden set (~35 units → ~7 locked-test) so `blocking` can flip true.
