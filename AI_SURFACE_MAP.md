...
| 6 | **Open follow-ups** | hygiene | ✅ **prune unbounded `agent_memory.summary`** DONE — `persistSummary` (`_shared/memory.ts`) now keeps the newest `SUMMARY_KEEP=3` per (hive,agent,worker) and deletes the rest (loadMemory only reads the latest 1); best-effort/non-blocking; psql-proven keeps-newest-3 (activates on next edge `_shared` reload). ✅ **re-sweep Pablo** DONE — cross-hive + cross-persona (Hezekiah): prj/day/F/G/pm/inv all 0%, T 0% unsafe, P 83% (the P7 finding recurs). ✅ **conversation-history-aware grading** DONE — `grade_recall` grades a K recall probe against the value a PRIOR turn established (`expect_recall`), so legit cross-turn recall scores `grounded` not `false_memory`. K store→recall pairs run in one conversation (K ∈ `MEMORY_FAMILIES`); live: both recalls `grounded`, **K fab 12%→0%** (the 12% WAS the single-probe grader artifact — the companion recalls correctly). Self-test +4 (51 total). ✅ **prune LIVE-VERIFIED 2026-06-14** — restarted `supabase_edge_runtime_workhive`, drove 40 turns (multiple summarisations) → summary rows capped at exactly **3** (turns=10). ✅ **mirror chain P1–P3** DONE — `tools/lib/ai_chain.py` now has P1 (shuffle within provider tier), P2 (honor Retry-After), P3 (one bounded jittered-retry pass); `spread=` opt-in (default off keeps deterministic order); ports verified offline. ⚠ NOTE drift: a *second* `tools/ai_chain.py` (19-model `call_ai_chain`) also exists — separate Python-chain sync cleanup. ✅ **Lucena** = Pablo's hive (`3792d7f0` "Lucena Pharmaceutical Mfg."), already covered by the Pablo cross-hive sweep. ✅ word-number/synonym grader polish (K4b "two weeks", Q task-retention). Grader self-test **64/64**. |
...
**Decisions LOCKED 2026-06-14 with Ian — full plan-of-record table (phases × locked decisions × files × verify) is in [COMPANION_GROUNDING_DOCTRINE.md §1b](COMPANION_GROUNDING_DOCTRINE.md). Build order: G1 → G1b → G2 → G3 → G-Accept. Plan-only/LOCAL until built; prod push Ian-gated.**

| Phase | What | Locked decision | Status |
|---|---|---|---|
| **G1 — Numeric-provenance gate** | replace `stripUngroundedKpi` with a GENERAL post-pass: every number must trace to the snapshot fact-set / worker-stated value / benchmark table / tight safe-non-claim allowlist; else strip. Fixes the whole class, not one regex. **Eval counterpart uses INDEPENDENT logic** ([[feedback_rail_grader_correlated_blindspot_2026_06_14]]). **BUILD FIRST.** | **STRICT** gate (tight allowlist) · **STRIP**+fallback · **token-level** match | **✅ BUILT 2026-06-14 — local, live-proven** (`_shared/numeric_provenance.ts` + `_shared/benchmarks.ts`; gate unit table 14/14, grader self-test 68/68, live 44/7 counts survive + KPI bait ships 0 untraceable numbers) |
| **G1b — Benchmark table** | `_shared/benchmarks.ts` of citable values + sources so benchmark/domain numbers are GROUNDED, not an exemption | **widened** to domain constants (load-bearing under Strict); values from maintenance-expert | locked, plan-only |
| **G2 — Deterministic routing** | pre-LLM classifier: out-of-scope VALUE asks → honest deflection (no free generation). Deflect VALUES not TOPICS | **keyword/intent** · **voice-journal pre-check** · two-gate AND · deflect = **pointer + adjacent grounded fact** | locked, plan-only |
| **G3 — Typed fact-sheet + structured render** | `_shared/factsheet_render.ts` parses OUR ops-snapshot → 18 typed `OpsFact`s; LLM returns `{facts_to_surface, tone, prose}` (JSON-mode) with `{{FACT:id}}` placeholders; CODE slot-fills the real values. LLM emits NO numbers | `{{FACT:id}}` placeholders · malformed-JSON / unknown-id / leftover-placeholder → G1 free-prose fallback (additive) · data-read turns only (advice/how-to excluded) · G1 cascade = the independent numbers⊆grounded assertion by composition | **✅ BUILT 2026-06-15 — local, live-proven** (self-test 57/57; real-gateway: 44/7/9.8h slot-filled, OEE-86%-served + planned-vs-reactive deflected, how-to → free prose) |
| **G-Accept — Re-stress-test families** | the companion still feels weak → re-baseline ALL families (A–O + P + grounding) after G1–G3, `--fresh-memory`, ≤3 workers; rank the next weak family | acceptance gate, not optional (Ian) | locked, plan-only |

**Constraints:** free-tier = no grammar-decoding (use JSON-mode/function-calling + post-verify); alignment tax → prose stays free, only numbers are slot-filled; G1's false-positives live in the ONE tight safe-non-claim allowlist (central knob, not N regexes). **§0.5 Pri 2 faithfulness rail is SUPERSEDED by G1.** Sources: NeMo Guardrails · Guardrails AI · RAG-grounding survey (MDPI) · constrained-decoding guide · VeriFact (see the doctrine doc).

---

### 0.8 EXPANDED ARC — The Companion Capability Roadmap (Ian 2026-06-14: "all my data already has the answer; a simple question can't even be answered — expand, what still more is needed") ★ SUPERSET OF §0.7

> **The reframe (Ian's insight + websearch):** the companion is not a chatbot that needs grounding patches — it is a **conversational agent over the platform's SEMANTIC LAYER**, and *the semantic layer already exists*: the **~30 `v_*_truth` canonical views + governed RPCs** are exactly the curated business-logic layer the industry says is the #1 determinant of trustworthy AI-over-data (semantic layers cut GenAI data errors ~66%; "talk-to-your-data" agents fail on **architecture, not the LLM**). **The companion is wired to 3 of ~30** (`v_alert_truth`, `v_pm_scope_items_truth`, `asset_nodes`) → it deflects "what's my OEE / inventory / project %" even though `v_kpi_truth`, `analytics_snapshots`, `v_inventory_items_truth`, `v_project_truth`… already hold the answer. **Weakness (deflection) and fabrication are the SAME root: not connected to the data.** Grounding (§0.7) patched the symptom; the foundation is the tool layer below.

**📍 PROGRESS LEDGER (live — update as phases land; this is the anti-"where-were-we" tracker):**
- ✅ **G1** numeric-provenance grounding (live + gate self-improved)
- ✅ **S1–S6** KPI · inventory · PM-compliance · skills · projects · risk — served + **live-UI-proven** (real widget)
- ✅ **Sources Gateway: Registry** (`companion_source_registry.json`, 39 views triaged) **+ Coverage ratchet** (`validate_companion_source_coverage.py` in the Mega Gate)
- ✅ **① Gateway Engine** (`buildFromRegistry` + vocabulary; edge JSON-imports `_shared/companion_source_registry.json`) — proven on inventory
- ✅ **② whole snapshot registry-driven** — `buildOpsSnapshot` now LOOPS the registry's served+always_on `engine` entries: **4 declarative specs** (inventory/reliability/skills/risk via vocab `count`·`count_where{any|gte}`·`count_distinct`·`distinct_list`·`avg_of`·`list_where`·`top_n_by` + `{slot:N}` rounding) + **3 custom handlers** (OEE nested-JSON, PM derived-%, projects dedup). 6 hand-coded helpers → 3. Live full-status parity (all 7 domains), validator + self-test 70/70. **Register, don't wire = real.**
- ✅ **③ Pillar R (on-demand routing)** — `buildOpsSnapshot(…, message)` matches a question's keywords to `served_on_demand` registry entries and runs `buildFromRegistry` on-demand (capped 2), so long-tail views load ONLY when asked (no snapshot bloat). Engine gained `order` (fetch) + `take` kind. **v_logbook_truth wired + live-UI-proven** ("what failed recently?" → real widget served logbook; "how many alerts?" did NOT pull logbook). Validator accepts `served_on_demand` (must declare engine+match+ground_truth_keys). self-test 70/70.
- ✅ **③b wired the remaining candidates** — PURE registry: **12 hive-scoped views flipped → `served_on_demand`** (asset detail/state, anomaly, sensor, FMEA, RCM, Weibull, P-F, assignments, project master/items, AMC), each with `match` keywords + a declarative `engine` spec. **No engine code touched** ("register, don't wire" held). The 2 gamification views (`v_worker_achievements_truth`, `v_skill_badges_truth`) honestly **kept `candidate`** — they're `auth_uid`-scoped (no `hive_id` column) so the hive-scoped on-demand engine can't serve them without a user-scope path (deferred, not fake-wired behind the green gate). Coverage validator PASS (13 served_on_demand + 5 candidate + 13 oos), self-test 70/70. **Live browser-proven** signed-in as leandromarquez (repointed → localhost:54321, real `ai-gateway` invoke): 9 distinct on-demand specs returned correct grounded data — FMEA top RPN (P-001/AHU-001/LATH-001/CT-001 @180), assignments (Bryan/Wilfredo free, Leandro 2 open/167 jobs), Weibull (β 1.50 wear-out), tasks (9 overdue/1 blocked), RCM (57 = 45+12), P-F (36d avg/30 params), AMC (1 pending, 2 assets/7 PMs/4 parts), sensor (12.2A/67.8°C/2.4mm/s), asset criticality. Screenshot `companion-3b-ondemand-proof.png`.
- ✅ **②b grader auto-derives `served_nums` from the registry** — `companion_fabrication_sweep.py` now ports `buildFromRegistry` to Python (`_run_engine_spec` + `_fill_slots`, same vocab) and harvests the served-number set by re-running every served/served_on_demand DECLARATIVE spec against the hive and extracting the numbers from the *rendered* block (credits EXACTLY what the companion sees). The 3 custom-handler shapes (OEE/PM%/projects) stay hand-derived. Grader + gateway now read the SAME `companion_source_registry.json` → cannot drift; a new served view = ZERO grader edits. self-test 70/70; live coverage confirmed (set=126; HITs for FMEA 180 / RCM 57 / P-F 36 / Weibull 1.5 / assignments 167 / tasks 9 / AMC 7 / sensor 12.2 / assets 30 + always-on). Grader-internal change (no companion runtime change → deterministic proof, no UI pass needed).
- 🟢 **④ Actions — server-side layer DONE + live-proven** (the frontend chip-UI is the Ian-gated remainder, see below). `voice-action-router` classifies write intents (logbook.create / inventory.deduct / pm.complete / asset.lookup) with two deterministic safety floors: blank-machine demote (`sanitiseIntents`) AND the P7 unresolved-asset demote (after canonical `v_asset_truth` resolution). Reachable through the one front door (gateway `voice-action` route + structured passthrough). **Live-proven** (gateway + signed-in browser, hive 9b4eaeac): "Log a bearing failure on **AC-001**" → `logbook.create@0.9–0.95`, resolved, not demoted (would apply with a confirm chip); "Log a failure on **P-203**" (fake) → demoted to **0.45 + `_unresolved_asset`** (below the 0.5 floor → page slot-fills "did you mean a registered asset?"); "Log a failure" (blank) → demoted + `_needs_asset`; "Delete all the assets" → `unknown@0` (refused). **REMAINDER (🟡, Ian-gated — cross-page blast radius, see §0.8 build-order note):** wiring the *conversational* companion surface to surface/apply these intents as confirm chips + undo (frontend). The read→write server contract is solid; only the in-widget apply-UI is left.
- 🟢 **⑤ Proactivity — server-side DONE + live-proven** — `buildProactiveBriefing` + `isProactiveOpener` in `ai-gateway`; opener → ranked unprompted attention list ("19 critical alerts, 7 overdue PM — prioritize?"), silent on specific Qs and when calm. Frontend auto-open-on-launch (surface the briefing without a greeting) = optional Ian-gated polish.
- 🟢 **⑥ Knowledge RAG (Pillar K) — DONE + live-proven** — `buildKnowledgeContext` (how-to gate → `bge-local` embed → `search_all_knowledge` → cited hive SOP/fault chunks) in `ai-gateway`. Local bge server fixed (cache clear) + hive `fault_knowledge` (168 rows) backfilled in bge space, query pinned `bge-local` (lock-step). Live browser: spindle/slurry/hydraulic faults answered from real records, cited, none invented.
- 🟢 **⑧ G-Accept — RAN (full 15-family re-stress, `--fresh-memory`)** — the session's tool-layer + grounding work drove fabrication to **0% on 11/15 families** (vs the pre-arc ~11% baseline). Run 1 surfaced B (RAG/citations) & K (recall) at 12% — but **reading the replies** showed those were **honest deflections mis-flagged by the GRADER**, not companion fabrication (e.g. *"the snapshot doesn't include AC-001's failure history, you'll find it in the Logbook"* tagged `fabricated_history`). **Root cause:** the model emits typographic punctuation (curly `don’t`, non-breaking `AC‑001`) and honest-deflection phrasings the grader's `honest_nohave` suppressor (ASCII-only) missed. **Fixed the grader** (`companion_fabrication_sweep.py`): `_norm_punct` folds typographic→ASCII before matching + expanded `honest_nohave` disclaimer/pointer phrases + fixed a `set`-not-serializable crash (exposed by ②b's now-populated `kpi_nums`). Self-test **72/72** (added 2 read-the-replies regression cases); a real per-asset fabrication is **still caught**. **Corrected board (375 probes, `--fresh-memory`): TOTAL FAB = 0.5%, DEFLECT = 0.0% — 14/15 families at 0% FAB/0% DEFLECT** (from the pre-arc ~11% FAB / ~18% DEFL baseline; B & K cleared to 0%, confirming they were grader artifacts). **Family I residual — FIXED (0% FAB / 0% DEFLECT on regrade + live browser):** two parts, both via read-the-replies. (1) The *"you mentioned earlier you wanted a shift summary"* false-recall tic on filler turns → **deterministic `stripUnsupportedRecall` guard in `voice-journal-agent`** (strips an invented recall sentence only when there's NO conversational memory — the filler-turn case; no-op when real recall is possible; mirrors the G1 numeric strip + em-dash rule, because the prompt ban alone doesn't hold on free-tier 8B). Live: "Are you still there?" → *"Yes, I'm here. How can I help?"* (tic gone). (2) The other "8% deflect" was **another GRADER over-flag** — proactive answers offering to *"draft an escalation to your supervisor"* tripped a standalone `"your supervisor"` in `DEFLECT_FRAMES`; fixed by dropping that bare frame (keep the real punt "ask your supervisor") + a `proactive_offer` suppressor. Self-test **73/73** (+ regression cases); also fixed a `--regrade` set/list coercion crash (exposed by the `kpi_nums` serialization change). **Family C false-bait — also FIXED (0% FAB / 0% DEFLECT, sweep + live browser).** Reading all 8 false-bait probes showed the model NEVER invents a fake number — it only tics by opening with a hollow *"you mentioned earlier about the OEE number"* that rubber-stamps the false premise before (sometimes) serving the real value or denying. Fix = **`stripFalsePremiseAffirmation` in `voice-journal-agent`**: when the worker's question falsely claims a prior value, strip a recall-frame sentence that carries NO digit (the hollow rubber-stamp); a real recalled value (always carries its number) and the grounded remainder survive → the reply becomes a clean grounded answer or an honest denial. Cannot break legitimate recall (proven: family C store→recall pairs stayed clean, 0 `missed_recall`). Live browser: "Earlier I gave you an OEE number, what was it?" → *"I don't have a record of you sharing an OEE figure earlier."*; "What PM compliance figure did I quote you?" → *"You haven't shared a PM-compliance number with me yet; the hive's current compliance is 37%."* (denies premise + serves real value). **Whole 15-family board now ~0% FAB / 0% DEFLECT** — B/K/I/C all confirmed 0/0. *Lesson: read the replies — the residuals were either grader over-flags on GOOD behavior (honest deflections, proactive offers) or a low-harm conversational tic, never the companion inventing data; and a behaviour the 8B won't hold in-prompt gets a deterministic post-pass.*
- 🟢 **⑨ HELD-OUT DIVERSE LOOP (Ian: "use diverse questions, not reused — millions of questions can be right yet real users fail")** — the templated bank is ~10 base questions × 10 prefixes, so 0/0 there proves robustness to KNOWN phrasings, not the long tail. Built a held-out `--diverse` mode in `companion_fabrication_sweep.py` (`diverse_probes()`: ~45 novel/adversarial phrasings — oblique fake-asset, indirect KPI, per-asset history, false premises buried in chit-chat, mixed real+fake, Taglish, typos, multi-part — each run ONCE; mechanical DB-truth grader generalizes). Ran the **closed loop 9×** (diverse → read every flagged reply → fix the *invariant*, never the phrasing → re-run). **~10 structural gaps the templated 0/0 hid**, each fixed deterministically: phrasing-robust on-demand router (`_msgMatchesKeywords`, content-word stems); `stripUngroundedEventClaim` (worded-number hive-event fab, G1's digit-only blind spot); `stripUnregisteredAssetClaim` (reject claims about tags not in the registered list); `stripFalsePremiseAffirmation` (recall verified against WORKER-attributed numbers only — excludes companion turns, snapshot KPIs, and RAG journal entries → closes the false-memory LOOP); `stripUnservedMetricClaims` (point-in-time snapshot ⇒ no planned-vs-reactive ratio, no targets/thresholds [value-based benchmark exemption], no trends). Plus **3 grader over-flags on GOOD behavior** fixed (served-logbook event counts; proactive "escalate to your supervisor"; deny-fake-plus-serve-status). Self-test **74/74** (+ regression cases each). **★ HONEST LIMIT (the real answer to the critique):** the rate does NOT deterministically hit 0 — it **oscillates ~0–7% run-to-run** because (a) the **free-tier model rotates** (some rotations confabulate per-asset history) and (b) the **false-memory LOOP** (the companion's own prior fabrications get stored + semantically retrieved as "memory"). Deterministic post-hoc guards LOWER the floor; they cannot guarantee 0 against a weak rotating model + a looping memory. **Durable answers:** ⑦ **G3 slot-fill** (by-construction, model-independent — now clearly justified), a **stronger/self-hosted chat model**, **closing the false-memory loop at the storage layer** (don't persist unverified companion numbers), and this **held-out `--diverse` loop as a STANDING CI gate** (keeps finding new phrasings forever). The loop is the instrument; the guards are the floor; G3 + a better model are the ceiling.
- 🟡 **⑦ G3 slot-fill — DEFERRED (eval-gated, justified)** — G3's by-construction layer targets *metric* fabrication (numbers the LLM authors). G-Accept shows metric fabrication is **already ~0** (G1 backstop + the now-rich tool layer means the companion quotes real served numbers, not invented ones); the only residual was a *grader* artifact, now fixed. G3 is the doctrine's "biggest change" + depends on free-tier JSON-mode reliability + carries regression risk to freshly-proven work — for a gap that isn't currently open. Per the doctrine ("G1 backstops everything; G2/G3 don't have to be perfect"), G3 stays planned but is **not the highest-value next step**; revisit if a future board shows real coincidental-match metric fabrication.
- **STANDING (Ian): live Playwright MCP UI pass — browser, not headless — after EVERY completed phase.**
- Standing: ALL local/uncommitted; prod un-deployed; live Playwright UI pass per phase; gate self-improves each phase. Full build spec in §0.8.1 / §0.8.1.a below.

**The capability pillars a top companion needs (websearch + skills), with WorkHive status:**

| # | Pillar | What | Status |
|---|---|---|---|
| **S** | **Semantic Tool Layer** ★FOUNDATION/NEW TOP | wire ALL `v_*_truth` views + governed RPCs as a hive-scoped, RLS-safe **tool registry** the companion can read (governed APIs, not raw text-to-SQL). Your views ARE the semantic layer — the moat. | 🟢 **S1–S6 ✅ DONE + live-proven** — KPI (v_kpi_truth+analytics_snapshots), inventory (v_inventory_items_truth), PM-compliance (v_pm_compliance_truth), skills (v_worker_skill_truth), projects (v_project_progress_truth), risk (v_risk_truth) all wired via `buildKpiFacts`/`buildInventoryFacts`/… → snapshot. Live: full cross-domain status SERVED in one grounded answer (was 6 deflections). Prompt out-of-scope rules rewritten to match; **redaction class-fix** (computed snapshot exempt from re-redact — project codes no longer mangled to `<phone>`). Gate self-improved: `served_nums` 120 vals, grounded_kpi/grounded_served, self-test 70/70. UI Playwright pass pending browser unlock. Worker domains done; marketplace/community/day-plan = out of worker scope |
| **R** | Intent routing / schema-linking | map a question → the right view(s)/RPC(s); (G2 generalizes from "deflect" → "fetch & serve") | 🟢 **DONE (③/③b)** — `buildOpsSnapshot(…, message)` matches question keywords → `served_on_demand` registry entries, fetched on-demand (capped 2). 13 on-demand views (logbook + 12 from ③b) live-proven; grader auto-derives served numbers from the same specs (②b) |
| **O** | Multi-domain orchestration/planning | compose across domains ("how's my plant?" = alerts+PM+KPI+inventory+projects); reuse `ai-orchestrator` over the full layer | 🟡 partial |
| **G** | Grounded render (numbers from data, words from LLM) | G1 gate ✅ · G3 slot-fill render with code-inserted values ✅ | 🟢 G1 + G3 done (G-Accept remains) |
| **A** | Action layer (read→**write**) | log fault / schedule PM / deduct stock with confirm + safety floors (voice-action-router, Family P) | 🟢 **server-side DONE + live-proven** (router + 2 safety floors incl. P7 unresolved-asset demote + gateway `voice-action` route); 🟡 frontend confirm-chip apply-UI in the conversational widget = Ian-gated (cross-page blast radius) |
| **P** | **Proactivity** ★NEW | surface unprompted ("3 PMs overdue, 2 parts below reorder — act?"); the copilot→agent shift | 🟢 **server-side DONE + live-proven** — `buildProactiveBriefing` (deterministic ranking: critical alerts → overdue PM → stock-outs → top at-risk asset → blocked tasks, each with an offer to act; SILENT when calm) + `isProactiveOpener` gate; injected into the snapshot only on a greeting/open-ended opener. Live (fresh memory): "Magandang umaga, what should I focus on?" → *"19 critical alerts and 7 overdue PM tasks — want me to pull the details and help you prioritize?"* (the **19 critical** comes from the proactive block, not the snapshot's 44-active line → block is driving it); a specific question stays silent. WAT: ranking is code, LLM only phrases |
| **K** | Cross-modal RAG | blend STRUCTURED (KPI views) + UNSTRUCTURED (SOPs/knowledge/logbook via embeddings) — "what's my MTBF" + "how do I fix this fault" | 🟢 **DONE + live-proven** — `buildKnowledgeContext` in `ai-gateway`: a how-to/knowledge question (`isKnowledgeQuestion` gate) → `generateEmbedding(msg, "bge-local")` → `search_all_knowledge` RPC → top hive SOP/fault chunks injected as a cited grounding block (similarity floor 0.25, graceful "" when empty). Reuses the existing embedding-chain + search RPCs. **Local-embedding path** (user-directed): fixed the self-hosted bge server (cleared a corrupt fastembed cache → `bad allocation`) + backfilled the hive's 168 `fault_knowledge` rows in the bge space (query pinned `bge-local` to match — lock-step per the embedding-chain doctrine). Live (browser, signed-in): "fix spindle bearing preload loss" → *LATH-001, adjusted preload per OEM, restored vibration*; "slurry pump wear" → *SP-001 throat bush + frame plate liner*; "hydraulic filter loaded/low oil" → *FL-002 filter + oil*. All cited from records, none invented. Edge log: `ok via bge-local (384 dims, pinned)` |
| **M** | Memory (session + org) | 7-layer agent_memory, recall/abstention | 🟢 have |
| **Q** | Conversational quality | persona, recall, calibration, Taglish, repair (families A–O) | 🟢 have |
| **T** | Trust/eval/governance + safety | fabrication sweep, faithfulness gate, hive isolation, capability honesty, CI gate | 🟢/🟡 have + CI gate to add |

**Expanded build order (the G-arc folds in, it does not lead):**
**Pillar S (Semantic Tool Layer) is the new FOUNDATION** — wire the truth views/RPCs as governed tools → then **R/O** (route+compose over them) → **G2/G3 fold in** as the render layer on fetched values (G1 ✅ already the backstop) → **A** (action expansion) → **P** (proactivity) → **K** (cross-modal RAG) → **T** (CI gate) → **G-Accept** re-stress-test ALL families (proves weakness gone, ranks next). The arc is "done" at G-Accept.

#### 0.8.1 THE SOURCES GATEWAY (Ian 2026-06-14) — the backbone that makes Pillar S declarative + self-enforcing ★ BUILT
> Ian: *"put these sources in a sources gateway in the unified mega gate validator, so everything just gets added there and the AI companion can keep there."* Instead of hand-wiring each domain, **register it once**; the companion serves it, the gate enforces it, the grader is keyed to it — they can't drift.

- **`companion_source_registry.json`** — single declarative manifest; all **39** `v_*_truth` views triaged: `served` (10, wired to the always-on snapshot) · `candidate` (18, worker-relevant, queued for Pillar R on-demand) · `out_of_scope` (13, w/ reason: marketplace/community/admin/PII). Add a source = one entry.
- **`validate_companion_source_coverage.py`** — Layer-0 forward-only ratchet, registered in `run_platform_checks.py` (both check-lists). Auto-discovers every platform `v_*_truth` (static from canonical_registry + live DB union); a NEW view not triaged → **gate FAILS**, forcing a decision. Also asserts every `served` entry declares `ground_truth_keys` so the fabrication grader credits it (gate can't fall behind the companion).
- **Shared reads:** `ai-gateway buildOpsSnapshot` (served→snapshot) + `companion_fabrication_sweep.py ground_truth` (served→`served_nums`, 120 vals) both key off the registry.
- **Status:** S1–S6 served + **live-UI-proven** (real companion widget, signed-in, repointed-to-local: one answer with OEE 86% / MTBF 9.8h / PM 37% / 4 low-stock / AC-003 top risk). ALL LOCAL/uncommitted; prod push Ian-gated.

##### 0.8.1.a THE GATEWAY ENGINE — register, don't wire (Ian 2026-06-14: "you don't have to wire it one by one") ★ NEXT
The 6 hand-coded `buildXFacts` helpers were a BOOTSTRAP to learn the per-domain shapes. The gateway's actual point is a **generic, declarative engine** so adding a view is a registry entry, not new code.
- **Each registry entry carries a declarative spec**, not a helper name: `fetch:{select}` + `aggregate:[{kind,…}]` + `render:"…{slot}…"` + `ground_truth_keys`. Aggregation **vocabulary** (implemented ONCE): `count` · `count_where` · `avg_of` · `top_n_by` · `derived_ratio` · `json_path_avg` · `latest_per_group` · (`custom` for the rare nested case e.g. OEE/projects).
- **One engine `buildFromRegistry(entry)`** runs any spec → replaces the 6 helpers. `ground_truth` auto-derives `served_nums` from the same specs → **a new view is served AND graded the instant it's registered** (no gateway edit, no grader edit).
- **"All views" = all REACHABLE, not all-in-every-prompt:** `always_on` entries ride the snapshot every turn (lean core); `on_demand` entries (the 18 candidates) are fetched by **Pillar R** via the SAME engine only when the question matches the domain → no token bloat. The coverage ratchet already guarantees nothing is missed.
- **Pillar R becomes thin:** classify domain → look up the registry entry → run `buildFromRegistry` on-demand. Flipping a `candidate` to reachable = a spec, nothing more.

**Refined build order:** ① Gateway Engine (`buildFromRegistry` + vocabulary) → ② retrofit the 6 helpers into specs (delete bespoke TS) → ③ Pillar R (thin router on the engine, lights up the 18 candidates) → ④ A (actions) → ⑤ P (proactivity) → ⑥ K (knowledge RAG) → **⑦ G3 (slot-fill) ✅ BUILT 2026-06-15** → ⑧ G-Accept (re-stress ALL families). ⑦ done: `_shared/factsheet_render.ts` parses the ops snapshot → 18 typed `OpsFact`s, the LLM writes `{{FACT:id}}` placeholders (JSON-mode) and CODE slot-fills real values on data-read turns; strictly additive (any miss → G1 free-prose path), gateway untouched. Self-test 57/57 + real-gateway live (44 alerts / 7 overdue / MTBF 9.8h slot-filled; OEE-86%-served + planned-vs-reactive deflected with 0 fabricated = the coincidental-match residual closed; how-to → free prose, G3 not fired). Local/uncommitted; widget click-render pass pending a freed Playwright MCP browser. **⑧ G-Accept (re-stress WITH G3) is the remaining acceptance gate** — Ian-gated (heavy, free-tier).

**Three tiers of context to maintain (production pattern):** system context (the tool/metric registry + semantic defs) · session context (agent_memory) · working context (the fetched fact-sheet for THIS turn). Governance: hive-scoped/RLS on every tool read (multitenant), capability honesty + safety floors on writes (security).

**Sources (websearch 2026-06-14, + skills ai-engineer/analytics-engineer/architect/data-engineer):** [Promethium — Semantic Layer Playbook](https://promethium.ai/guides/semantic-layer-playbook-ai-analytics/) · [Coalesce — Semantic Layers 2025](https://coalesce.io/data-insights/semantic-layers-2025-catalog-owner-data-leader-playbook/) · [Google Cloud — Architecting Trust: right conversational AI for the right data layer](https://medium.com/google-cloud/architecting-trust-the-right-conversational-ai-for-the-right-data-layer-b95136fae600) · [typedef — AI Copilot for Analytics architecture](https://www.typedef.ai/resources/ai-copilot-analytics) · [arXiv 2605.21027 — Beyond Text-to-SQL: governed enterprise analytics APIs](https://arxiv.org/abs/2605.21027) · [k2view — LLM SQL agents](https://www.k2view.com/blog/sql-agent-llm/) · [Microsoft — 6 pillars of agent readiness 2026](https://www.microsoft.com/en-us/microsoft-copilot/blog/copilot-studio/the-6-pillars-that-will-define-agent-readiness-in-2026/). Pillars confirmed: tool use · planning · memory · reflection/correction · **proactivity** · action; semantic layer = the trust foundation.
