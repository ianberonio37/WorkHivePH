#!/usr/bin/env python3
"""Layer 2 E2E: plant-connections.html — Enterprise settings, plant/site config, multi-tenant isolation."""
from playwright.sync_api import sync_playwright, Page
from e2e_helpers import E2ETestHelper

BASE_URL = "http://127.0.0.1:5000/workhive"
PAGE = "plant-connections"
CARD = ".plant-card, [data-plant-id], .site-row, .connection-card"
...
def test_read(page: Page) -> dict:
    ...
    results.append({"scenario": "happy", "result": "PASS" if count >= 0 and no_undef else "FAIL", "actual": f"{count} plants"})
    results.append({"scenario": "empty_state", "result": "PASS" if count >= 0 else "WARN", "actual": "plants or empty"})
...
def test_write(page: Page) -> dict:
    ...
    h.clear()
    edit_btn = h.page.locator("button:has-text('Edit'), button:has-text('Configure'), [data-action='edit']").first
    if edit_btn.is_visible(timeout=3000):
        edit_btn.click()
        h.fill_form({"#plant_name": "Updated Plant Name"})
        h.submit_form("button:has-text('Save'), button:has-text('Update')")
        results.append({"scenario": "happy_update_config", "result": "PASS", "actual": "config updated"})
    else:
        results.append({"scenario": "happy_update_config", "result": "WARN", "actual": "no edit button"})
    ...
    add_btn = h.count_rendered_items("button:has-text('Add Plant'), [data-action='add-plant']") > 0
    results.append({"scenario": "permission_add_plant", "result": "PASS" if (role in ("supervisor", "admin")) == add_btn else "WARN", "actual": f"role={role}"})
...
def run(page: Page, errors: list, warnings: list, log=print) -> dict:
    ...
    log(f"Plant-Connections: {p} PASS / {f} FAIL / {w} WARN")
