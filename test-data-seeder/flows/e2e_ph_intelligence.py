#!/usr/bin/env python3
"""Layer 2 E2E: ph-intelligence.html — PH-specific insights, data freshness, error fallback."""
from playwright.sync_api import sync_playwright, Page
from e2e_helpers import E2ETestHelper

BASE_URL = "http://127.0.0.1:5000/workhive"
PAGE = "ph-intelligence"
CARD = ".insight-card, [data-insight-id], .kpi-card"
...
def test_read(page: Page) -> dict:
    ...
    results.append({"scenario": "happy", "result": "PASS" if count >= 0 and no_undef else "FAIL", "actual": f"{count} insights"})
    results.append({"scenario": "empty_state", "result": "PASS" if count >= 0 else "WARN", "actual": "insights or empty"})
...
def test_write(page: Page) -> dict:
    ...
    results.append({"scenario": "happy", "result": "SKIP", "reason": "read-only intelligence page"})
...
def test_additional(page: Page) -> dict:
    ...
    h.clear()
    fresh = h.verify_data_rendered("updated") or h.verify_data_rendered("last") or h.verify_data_rendered("ago")
    results.append({"scenario": "data_freshness", "result": "PASS" if fresh else "WARN", "actual": f"freshness_shown={fresh}"})
...
def run(page: Page, errors: list, warnings: list, log=print) -> dict:
    ...
    log(f"PH-Intelligence: {p} PASS / {f} FAIL / {w} WARN")
