...
def test_write(page: Page) -> dict:
    ...
    h.login()
    h.goto(PAGE)
    # Close any auto-opened wizard modal (project-manager opens wizard on load)
    h.page.wait_for_timeout(1200)
    h.page.evaluate("""() => {
        if (typeof closeModal === 'function') {
            closeModal('modal-wizard'); closeModal('modal-project');
        }
        ['modal-wizard', 'modal-project', 'modal'].forEach(id => {
