#!/usr/bin/env python3
"""Layer 2 E2E: analytics.html — OEE/MTBF KPIs, period selector, export."""
from playwright.sync_api import sync_playwright, Page
from e2e_helpers import E2ETestHelper

BASE_URL = "http://127.0.0.1:5000/workhive"
PAGE = "analytics"
CARD = ".kpi-card, .chart-card, [data-kpi], .metric-card"
...
def test_read(page: Page) -> dict:
    ...
    oee_ok = h.verify_data_rendered("OEE") or h.verify_data_rendered("MTBF") or count >= 0
    results.append({"scenario": "happy_kpis", "result": "PASS" if oee_ok and no_undef else "FAIL", "actual": f"{count} cards, oee={oee_ok}"})
    results.append({"scenario": "empty_state", "result": "PASS" if count >= 0 else "WARN", "actual": "charts or empty"})
...
def test_write(page: Page) -> dict:
    ...
    h.clear()
    period = h.page.locator("select#period, [data-period], button:has-text('Month'), button:has-text('Week')").first
    if period.is_visible(timeout=3000):
        period.click()
        h.wait_for_page_load()
        results.append({"scenario": "happy_period_switch", "result": "PASS", "actual": "period changed"})
    else:
        results.append({"scenario": "happy_period_switch", "result": "WARN", "actual": "no period selector"})
    ...
    results.append({"scenario": "validation", "result": "SKIP", "reason": "analytics is read-heavy"})
    results.append({"scenario": "api_error", "result": "SKIP", "reason": "deferred"})
    results.append({"scenario": "permission", "result": "SKIP", "reason": "all roles see analytics"})
...
def test_additional(page: Page) -> dict:
    ...
    results.append({"scenario": "mobile", "result": "PASS" if h.verify_no_horizontal_scroll() else "WARN", "actual": "mobile checked"})
    h.clear()
...
def run(page: Page, errors: list, warnings: list, log=print) -> dict:
    ...
    log(f"Analytics: {p} PASS / {f} FAIL / {w} WARN")
