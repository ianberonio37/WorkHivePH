#!/usr/bin/env python3
"""Layer 2 E2E: symbol-gallery.html — Drawing symbols, filter by standard (IEC/NFPA)."""
from playwright.sync_api import sync_playwright, Page
from e2e_helpers import E2ETestHelper

BASE_URL = "http://127.0.0.1:5000/workhive"
PAGE = "symbol-gallery"
CARD = ".symbol-card, [data-symbol-id], .symbol-item"
...
def test_read(page: Page) -> dict:
    ...
    std_shown = h.verify_data_rendered("IEC") or h.verify_data_rendered("NFPA") or count >= 0
    results.append({"scenario": "happy", "result": "PASS" if std_shown and no_undef else "FAIL", "actual": f"{count} symbols, std={std_shown}"})
    results.append({"scenario": "empty_state", "result": "PASS" if count >= 0 else "WARN", "actual": "symbols or empty"})
...
def test_write(page: Page) -> dict:
    ...
    h.clear()
    filter_sel = h.page.locator("select#standard, [data-filter='standard'], button:has-text('IEC')").first
    if filter_sel.is_visible(timeout=3000):
        filter_sel.click()
        h.wait_for_page_load()
        results.append({"scenario": "happy_filter_standard", "result": "PASS", "actual": "filter applied"})
    else:
        results.append({"scenario": "happy_filter_standard", "result": "WARN", "actual": "no standard filter"})

    h.clear()
    search = h.page.locator("input[type='search'], input#search").first
    if search.is_visible(timeout=2000):
        ...
        results.append({"scenario": "happy_search", "result": "WARN", "actual": "no search"})

    results.append({"scenario": "validation", "result": "SKIP", "reason": "read/design-time only"})
    results.append({"scenario": "api_error", "result": "SKIP", "reason": "static gallery"})
    results.append({"scenario": "permission", "result": "SKIP", "reason": "all workers can browse"})
...
def run(page: Page, errors: list, warnings: list, log=print) -> dict:
    ...
    log(f"Symbol-Gallery: {p} PASS / {f} FAIL / {w} WARN")
