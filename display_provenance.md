...
- **E3 residual (resolved but not yet trustworthy):** 7 — the B.2 hardening backlog
...
### Data value — needs formula-grade element-id→query bind — 6
...
| marketplace-seller.html | `cw-total` |
| achievements.html | `ac-provider-tier` |

### UI chrome (control/label/counter) — EXCLUDE, no data provenance — 1

_E3: this element-id is UI CHROME (control/label/counter), not a data display — EXCLUDE from the data denominator (no provenance to bind)._

| Page | Element |
|---|---|
| community.html | `reply-char-count` |
