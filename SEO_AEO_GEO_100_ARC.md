...
- **`tools/orphan_depth_gate.py`** (P1 Crawl/index "0 orphans + ≤3 clicks" cell, BUILT 2026-06-30) — builds the internal-link graph from the SCRIPT-STRIPPED DOM (crawler view, reusing landing_extractability's model) over the catalog-derived public surfaces; BFS from index.html. Two ratcheted checks: `orphans` (no crawlable inbound `<a href>`) + `click_depth` (linked but unreachable, or >3 clicks). Self-test 14/14, registered (`orphan-depth`, AI Validation, skip_if_fast). **Live result: 0 orphans · 0 depth-violations — VERIFIED healthy** (43 surfaces, 199 edges, all reachable, max depth 2; the P2.5a catalog mirror + learn hub + footer already give a clean crawl graph). Baseline locked 0/0.
- **`tools/cwv_gate.py` + `tools/cwv_probe.mjs`** (P3 Core Web Vitals cell, BUILT 2026-06-30) — chose **Playwright over Lighthouse** (the perf-skill's proven honest path; toolchain has PW 1.60 + Chromium 148 ready, so Lighthouse would be a redundant install). Two-tool scorer: `cwv_probe.mjs` drives headless MOBILE Chromium across the same 43 catalog-derived public URLs (nav-start buffered observers, TRUSTED `mouse.click` for INP with CLS frozen first, median-of-3, **WARM-cache** measured LCP = code-intrinsic + a `lcp_cold_ms` first-visit field), writes `cwv_measurements.json`; `cwv_gate.py` ratchets it (LCP≤2500/INP≤200/CLS≤0.1, + a `coverage` check so a partial probe can't earn a free pass). Self-test 13/13, INP-unmeasured stamped not pass-credited, env:"local"/lcp_local_optimistic surfaced. **Measured result (warm mobile, median-of-3): LCP all GREEN (~0.5-1.2s) · CLS all GREEN (0) · INP GREEN.** (The first sweep flagged 4 pages at INP 368-896ms; a per-page diagnostic proved these were **measurement artifacts** — INPUT DELAY from clicking ~1500ms after load while floating-ai.js/Supabase/the feedback-FAB were still executing the main thread, on a *no-handler* `<h1>` the probe picks first-in-DOM. Re-measuring the same click settled = 16-24ms. Fix was to the PROBE methodology — `waitForLoadState('networkidle')` + a longer settle before the trusted INP click, the lab convention — NOT a frontend change. index.html 600→16ms confirmed; re-baselined to inp_over 0.) The separate cold-LCP 3-5s is the render-blocking Tailwind CDN first-visit (a known perf-track item, recorded as `lcp_cold_ms`).
