...
OPTIONAL_VARS: dict = {
    ...
    "RESEND_WEBHOOK_SECRET": "Resend bounce/complaint webhook (resend-webhook-receiver) -- the Svix signing secret. UNSET MEANS THE RECEIVER FAILS CLOSED: index.ts:88 reads it as `|| ''`, so every signature mismatches and every event is rejected 401. Bounces are not ingested and send-time failures still report exactly as they do today; nothing throws. Fail-closed is deliberate -- an unverified webhook body is attacker-controlled input that could mark real contacts as bounced.",
    "GCASH_INBOUND_SECRET": "mig 38 GCash receipt webhook (gcash-receipt-inbound) — the HMAC-SHA256 shared secret a forwarder signs its POST with. UNSET MEANS THE ENDPOINT REFUSES EVERY REQUEST, which is the intended degrade and the reason it belongs here rather than in .env: with no secret there is no way to tell a forwarded receipt from a forged one, and a forged receipt would auto-verify a top-up and mint credits nobody paid for. Failing closed loses only the automation; the founder's manual queue in platform-actions still verifies every top-up by hand. Set in cloud secrets only, and give the same value to the forwarder. Declared for humans in .env.example too.",
