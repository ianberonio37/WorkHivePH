...
| D8 | **Extract a shared page-init helper** (`initWorkHivePage()`) — surfaced by D7 | extract the ~15–25-line SUPABASE consts + `getDb` + `logPageView` + `restoreIdentityFromSession` + WORKER_NAME/HIVE_ID boilerplate repeated on ~38 pages / leave as-is | **NOT committed — its own careful effort.** The GENUINE dedup the D7 clone-debt rise exposed (the auth/identity boilerplate is now a clean contiguous clone across ~38 pages); collapsing it would bring clone-debt back below 4742. BUT behavior-changing (auth redirects, identity restore, per-page variations) → per-page care + live proof; higher-risk than D7. |
...
### The fork for Ian (D5)
Group C is the **component-dedup spine** Ian approved. The skills are clear that the single *most valuable* thing still un-streamlined is the **AI-display fragmentation** — the other half of the charter. It's deliberately left as **D5 (surfaced, not committed)**: say the word and it opens as a parallel arc; otherwise Group C stands alone. Either way — **plan-only until the build-go (D6).**
