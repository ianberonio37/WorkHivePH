# RAG Flywheel — Turn 39 report
Generated: 2026-05-23T10:12:16.661164+00:00
...
- Routes used      : {'semantic': 65, 'n/a': 18}
- Avg latency      : 3669.9 ms
- Avg tokens       : 410.4

## Convergence metrics
- Grader pass rate    : **16.9%** (14/83)
- Checker pass rate   : **7.2%** (6/83)
- Citation coverage   : 16.9% (14/83 tiles had ≥1 citation)

## Gaps found
- Tiles missing canonical anchor      : **0**
- Tiles with checker FAIL (need work) : **77**
- Tiles with zero citations           : **51**

## Auto-actions taken
- canonical_sources INSERTed : **0**
- New L0 tile locks added    : **0**
- L2 review queue            : `.tmp/flywheel_turn_39_l2_review.md` (manual review per locked decision)
...
- `asset-hub::asset-hub:rcm_edges` — value= — AI answer: _The 'Asset RCM edges' KPI measures reliability-centered maintenance (RCM) strategies per SAE JA1011 decision tree, outpu_
- `pm-scheduler::pm-scheduler:overdue` — value=0 — AI answer: __
- `pm-scheduler::pm-scheduler:on_track` — value=13 — AI answer: _I don't have enough recent records to answer that. Try rephrasing or asking about a specific asset / date._
...
- `skillmatrix::skillmatrix:on_target` — value=5/5 — AI answer: _the number reflects workers who have met or exceeded their skill-count target [def#v_worker_skill_truth_on_target]. The _
...
- `achievements::achievements:total_level` — value=71 — AI answer: _the canonical source is listed in [def#v_worker_achievements] [def view v_worker_achievements]_
...
- `achievements::achievements:composite_score` — value= — AI answer: _the canonical source is listed in def#v_worker_skill_truth_
...
- `integrations::integrations:active` — value=0 — AI answer: _The 'Active integrations' tile shows a non-zero value when there are active worker reservations, parts staged reservatio_
...
- `report-sender::report-sender:detail_panel` — value=4 rows — AI answer: _The report-sender tile counts how many reports the user has ticked to include in the next send, sourced from v_report_se_
- `shift-brain::shift-brain:top_risk_this_shift` — value=0 — AI answer: _the canonical source is listed in [def#v_risk_truth]. This tile would show a non-zero value when a risk score is calcula_
