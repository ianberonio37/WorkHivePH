...
## Next layer — Derive-and-Render + Self-Running Loop + Flagship Grounding  [added 2026-06-29]

**Status:** PLAN AGREED (forks locked by Ian 2026-06-29). Build not started. Read this section first on resume — it supersedes the "build the gate" framing; the gate IS built, this layer closes the loop's three leaks.

### Why this layer (the 2026-06-29 state map)
A 4-reader ground-truth sweep (`content-grounding-state-map` workflow) found the loop is built but **leaking in three places** — which is exactly the "stale / not aligned" feeling, just NOT where it was assumed to be:

- **The learn articles are the HEALTHIEST surface** (born-grounded, gate-clean at last run, 96.4% feature coverage, 0 drift). The "my articles are outdated" instinct points at the wrong surface.
- **Real drift is concentrated in the still-hand-maintained / born-outside surfaces:**
  - `index.html` **self-contradicts**: line 1893 "24 in-depth guides" vs line 1921 "38 guides" — both hardcoded, nothing reconciles them. Catalog *documents* this but by design never *writes* index.html.
  - `llms.txt` hand-kept + drifted: lists 37 articles vs 38 in catalog (missing the AI-companion-personas article).
  - `learn/index.html` hub: claims 37 guides, links 38, "Last updated 16 May 2026".
  - **The gate is a SENSOR, not an ACTUATOR, and last ran 2026-06-12** (17 days stale at time of plan). Platform changed since → drift accumulating uncaught; "all green" is a stale snapshot.
  - **Flagship video pipeline (built 2026-06-29) is born OUTSIDE the fence:** `cmd_flagship`/`render_flagship.py`/`FlagshipReel.tsx` get only `PLATFORM_CONTEXT`, no page-evidence, and NO capability gate runs on flagship specs (scripts have both). `storyboard.py` still carries the 21 hardcoded `FEATURE_KEYWORDS`/`FEATURE_NAME_TO_KEY` maps the original plan said to replace.

### The three leaks (the diagnosis this layer fixes)
1. **The loop doesn't run itself** → drift accumulates between manual gate runs.
2. **Three surfaces are sense-only, never healed** → landing page / llms.txt / hub are flagged-but-never-fixed (and currently not even flagged, gate stale).
3. **New formats are born ungrounded** → each new surface (flagship video) bypasses the gate unless explicitly wired; the newest one did.

### Locked decisions (forks, Ian 2026-06-29)
- **D-strategy = Derive-and-render the FACTS.** The structural/factual parts of the hand-maintained surfaces (counts, article lists, feature lists, JSON-LD `featureList`) become **generated from the catalog** via a build step, so they CANNOT drift — the inward `v_*_truth` move pointed outward. Prose/voice stays hand-written. (Hybrid in spirit: facts auto-derived, prose draft-then-dispose.)
- **D-cadence = BOTH** (a) fold `phase_content` into the default `run_platform_checks` (every gate run checks content drift, not just `--with-content`); AND (b) a scheduled **cron sweep** (`content_dev.py mega`) to catch drift-over-time even with no commit.
- **D-flagship = pull it inside the fence.** Ground flagship spec generation in page-evidence + catalog (mirror `_evidence_block_for_feature`) and run `capability_issues_for_text` on the generated spec, gated like scripts.

### Build sequence (each verifiable)
- **D0 — Re-baseline. ✅ DONE 2026-06-29.** Rebuilt substrate against today's platform + re-ran the gate. Found exactly ONE live drift: `feature_drift` (article `gamifying-maintenance-for-engagement` + `llms.txt` "Maps to Achievements", which the catalog no longer resolved after STREAMLINE F5 folded Achievements + Predictive Analytics out of the top-level nav). **Heal (decision: keep-as-feature/resolve-folded):** `platform_catalog.py` now has a **folded-feature pass** — a marketed intel feature whose page still EXISTS on disk but is no longer a nav tool becomes an active `nav_status:"folded"` feature (catalog = capability truth, nav = presentation). Generic → covered both Achievements AND Predictive Analytics; **zero collateral** (no page/article/landing edit; coverage stayed 100%, no new gaps; the predictive-maintenance article correctly re-homed to Predictive Analytics). Result: `feature_drift` 1→0, gate 10/10, **mega PASS, all 4 axes 100%**, 2 `alias_route_missing` notes cleared. **Two tool-robustness bugs found + fixed along the way:** (1) `content_grounding_gate.py` self-test "strict mode FAILs while real drift exists" was **live-state-dependent** — it broke the moment the last real drift was healed; rewritten to use SYNTHETIC drift (live-state-independent teeth). (2) `count_drift`'s count regex **misses "N in-depth guides"** phrasing — it reported `guide:[38,38]` and never caught the real `index.html` contradiction (line 1893 "24 in-depth guides" vs line 1921 "38 guides"); to be hardened in D1.
- **D1 — Derive-and-render the facts. SCOPE = FULL regeneration (Ian 2026-06-29):** counts, lists, prose feature-names, JSON-LD `featureList`, AND FAQ/HowTo all rendered from the catalog. Marker syntax: `<!--CATALOG:key-->value<!--/CATALOG:key-->` (HTML comment — invisible in-browser, harmless as text in `.txt`). The engine only ever rewrites bytes BETWEEN a marker pair; prose outside is untouched; idempotent.
  - **D1.1 ✅ DONE 2026-06-29 — foundational slice.** Built `tools/render_public_surface.py` (extensible `render_values()` map; `--check`/`--apply`/`--print`/`--self-test`, synthetic live-state-independent self-test). Wired `guide_count` markers into `index.html` (both the line-1893 "in-depth guides" claim and the line-1921 "all guides" link). Demonstrated check (caught 24≠38, exit 1) → apply (healed to 38) → check (clean): **the 24-vs-38 contradiction is fixed by derivation and locked.** Added gate check #11 `surface_render_drift` (ratcheted, baseline 0) — a hand-edit to a derived region now FAILs the gate. Hardened `count_drift`'s regex (it had missed "N in-depth guides" — the very bug it should have caught) with a bounded marketing-adjective allow-list. **mega PASS, 11/11 checks, all 4 axes 100%.**
  - **D1.2–D1.5 (remaining full-regen increments):** llms.txt article-list membership + count (fixes the 37-vs-38 / missing ai-companion-personas; preserve curated descriptions, derive membership) + the learn-hub count/date; JSON-LD `featureList` rendered from catalog; prose feature-name lists (e.g. line 1893's topic list) checked/derived vs catalog; FAQ/HowTo JSON-LD blocks rendered from the catalog FAQ set.
- **D2 — Make the loop self-running. ✅ DONE (fold) 2026-06-29.** Registered the Content Grounding Gate as a DEFAULT entry in `run_platform_checks.py`'s `VALIDATORS` registry (`id: content-grounding` → `tools/content_grounding_gate.py`, group "AI Validation", ratcheted, ~1.8s offline). Verified through the real orchestrator runner: **STATUS PASS, 12/12 checks.** So content↔platform drift is now caught on *every* routine gate run — the sensor that went 17 days stale is self-running. **Cron half (belt-and-suspenders, Ian-gated OS schedule):** the sweep entrypoint already exists — `python tools/content_dev.py mega`. Register a daily Windows task when wanted (reversible via `schtasks /delete`):
  `schtasks /create /tn "WorkHive Content Sweep" /tr "cmd /c cd /d \"<repo>\" && python tools/content_dev.py mega" /sc daily /st 07:00`
  (Not auto-installed — a persistent background process on Ian's machine is his opt-in; the default-gate fold already provides self-running on every run.)
- **D3 — Pull flagship inside the fence. ✅ DONE 2026-06-29.** `cmd_flagship` now injects the feature's REAL page-evidence affordances into the spec prompt (`_evidence_for_feature`) AND runs `capability_issues_for_text` on the generated spec's captions + data callouts (born-grounded + gated, like scripts/articles). **Proven end-to-end live:** a real gen (groq/llama-4-scout via the ai_chain) produced a spec that passed `capability-grounding: OK`; offline the gate flags an invented control (teeth) + passes grounded copy (no false positive). **storyboard.py:** evidence proved the "kill the 21 maps" plan WRONG — the maps encode a real distinction the catalog collapses (`shift_handover` → Logbook feature, since handover has no standalone page), so a naive catalog-derive would lose the "Shift Handover Report" label. Correct move = a **drift GUARD** not a delete: a `--self-test` asserts every journey key (the load-bearing part that drives the recorder) still resolves to the live catalog; the curated narration regexes + label distinction stay. (Same "evidence overrides the stated plan" lesson as the Achievements fork + the curated-FAQ call.) **Bonus bug fixed:** `python tools/video_idea_generator.py` (its own documented CLI) failed on `from tools.* import` because a direct run puts tools/ on the path, not the repo root — added the repo-root `sys.path` insert.
- **D4 — Auditable provenance (optional/lower-priority, from the gap).** Emit a visible "Grounded in: `<page>`" trace on generated artifacts so a later hand-edit that loses grounding is catchable post-hoc (today provenance is generation-time only, leaves no trace).

### Definition of done (this layer)
A platform change (rename/add/remove a feature, ship/retire an article) → **without a human hand-editing**: the catalog re-derives; the landing page / llms.txt / hub FACTS re-render automatically (cannot drift); every prose/script/flagship surface that referenced the change is flagged + queued for dispose; the 4-axis scorecard dips; after regen the gate is green again — **and this runs on every default gate pass + on the cron schedule, not only when manually invoked.**
