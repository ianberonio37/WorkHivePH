...
| **FB2 — Browser-CI harness** *(#7 — biggest single live lever)* | Stand up the headless Playwright CI run that unblocks Arc D's ~811 strict-live cells + render cells. | All 4 pool personas walk every page headless; github MCP wires the CI job; ratchet the live% column. |
...
- **This doc: DRAFTED.** No stream started — execution is for the fresh window.
- **Fresh window opens at:** `build FB1` (Forward-Build served-edge live round-trips) — or, if a denominator-mine is wanted first, `study FB0`.
- **Pre-existing spines to reuse:** `CROSS_ARC_UFAI_REVIEW.md §3` (Stream 1), `SECURITY_ADVERSARIAL_ROADMAP.md` (Stream 2).
- **Spines to draft at window-open:** `ACCESSIBILITY_UFAI_ROADMAP.md` (U), `OBSERVABILITY_SLO_ROADMAP.md` (T).
