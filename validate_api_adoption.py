"""
API-Adoption Ratchet (L0) — FULLSTACK_COMPONENT_LIBRARY_ROADMAP Layer A, A-P2.
================================================================================
...
index.ts (never a stale report):
  ...
  A1  ADOPTION FLOOR (FAIL) — live adopter count >= the floor in
      api_adoption_baseline.json for every measured row. A function dropping
      its cors/envelope/rate-limit/tenant-context import is a regression.
...
Output: api_adoption_gate_report.json (+ rewrites api_adoption_baseline.json on
...
from api_adoption_census import run_census  # noqa: E402

BASELINE = ROOT / "api_adoption_baseline.json"
REPORT = ROOT / "api_adoption_gate_report.json"
...
def main() -> int:
    ...
    if not BASELINE.exists():
        ...
        print(f"api-adoption: first run — baseline created ({len(measured)} floors). PASS")
        return 0
    ...
    print(f"api-adoption: {len(measured)} measured rows"
