-- 20260825000006_pending_parts_created_at.sql  (T20, 2026-08-26)
-- The approval queue's part cards could not say WHEN a part was submitted: the RPC bundle's
-- pending_parts select omitted created_at (pending_assets carried it), so the queue's new
-- submitted-at line rendered only on the non-RPC fallback path. One column added; otherwise
-- a verbatim re-create of phase3's definition (the function's latest prior version).
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'pending_parts', CASE WHEN v_is_sup THEN COALESCE((SELECT jsonb_agg(p) FROM (
        SELECT id, part_number, part_name, qty_on_hand, category, submitted_by, worker_name, status, created_at
