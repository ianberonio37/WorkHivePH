...
(function () {
  ...
  function whCreateQueue(opts) {
    ...
    async function enqueue(item) {
      ...
      try { channel && channel.postMessage({ type: 'enqueue' }); } catch (_) {}
      return item;
    }
    ...
    async function remove(id) {
      ...
      try { channel && channel.postMessage({ type: 'drain' }); } catch (_) {}
    }

    async function clear() {
      const idb = await _idb();
      await _idbReq(idb, cfg.store, 'readwrite', (s) => s.clear());
      try { channel && channel.postMessage({ type: 'clear' }); } catch (_) {}
    }
    ...
    async function drain(supabase) {
        ...
        for (const item of pending) {
          ...
          if (!error) {
            ...
            if (typeof cfg.onSyncedRow === 'function') {
              try { cfg.onSyncedRow(item); } catch (_) {}
            }
          } else {
            errors += 1;
          }
      ...
      if (drained && typeof cfg.postSync === 'function') {
        try { await cfg.postSync({ drained, errors }); } catch (_) {}
      }
