# Triage — rerun_20260525_015919

- stats: {'startTime': '2026-05-24T13:59:31.205Z', 'duration': 3786397.842, 'expected': 535, 'skipped': 21, 'unexpected': 94, 'flaky': 0}
- total failures: **94**
  - A. Network / flake:        **3**
  - F. Fixture sign-in:        **0**
  - B. Sentinel content drift: **0**
  - C. Real regressions:       **91**

## Top failing files
| Count | File |
|---|---|
| 22 | `journey-canonical-signal-parity.spec.ts` |
| 13 | `journey-voice-journal.spec.ts` |
| 7 | `assistant.spec.ts` |
| 6 | `journey-asset-hub.spec.ts` |
| 5 | `journey-hive.spec.ts` |
| 5 | `journey-predictive.spec.ts` |
| 4 | `founder-console.spec.ts` |
| 4 | `journey-analytics.spec.ts` |
| 3 | `journey-skillmatrix.spec.ts` |
| 3 | `ph-intelligence.spec.ts` |
...
### `journey-canonical-signal-parity.spec.ts`  (22)
...
- L309 — check_low_stock_count: home tile == COUNT(v_inventory_items_truth WHERE is_low_stock)
  - err: `Error: Could not query canonical view v_inventory_items_truth.is_low_stock from page scope — db client may not be initialized. expect(received).toBeGreaterThanOrEqual(expected) Expected: >= 0 Received: -1 371 | `Could no`
- L309 — check_open_jobs_count: home tile == COUNT(v_logbook_truth WHERE status=Open)
  - err: `Error: Could not query canonical view v_logbook_truth.undefined from page scope — db client may not be initialized. expect(received).toBeGreaterThanOrEqual(expected) Expected: >= 0 Received: -1 371 | `Could not query can`
- L309 — check_risk_alerts_count: home tile == COUNT(v_risk_truth WHERE risk_level IN critical/high)
  - err: `Error: Could not query canonical view v_risk_truth.undefined from page scope — db client may not be initialized. expect(received).toBeGreaterThanOrEqual(expected) Expected: >= 0 Received: -1 371 | `Could not query canoni`
- L309 — check_pm_overdue_home_tile: home tile == hive Team Pulse count (both v_pm_scope_items_truth.is_overdue / v_pm_compliance_truth.is_due rollup)
  - err: `Error: Could not query canonical view v_pm_compliance_truth.is_due from page scope — db client may not be initialized. expect(received).toBeGreaterThanOrEqual(expected) Expected: >= 0 Received: -1 371 | `Could not query `
...
### `founder-console.spec.ts`  (4)
- L15 — admin_gate_present: page declares an admin gate function
  - err: `ReferenceError: pageSrcWithExternals is not defined 16 | await whPage.goto(PAGE); 17 | await waitForPageReady(whPage); > 18 | const __sentSrc = await pageSrcWithExternals(whPage); | ^ 19 | const has = /isPlatformAdmin|is`
- L23 — admin_gate_active: admin gate is not commented out / stubbed
  - err: `ReferenceError: pageSrcWithExternals is not defined 24 | await whPage.goto(PAGE); 25 | await waitForPageReady(whPage); > 26 | const __sentSrc_2 = await pageSrcWithExternals(whPage); | ^ 27 | const commented = /\/\/\s*(if`
- L31 — admin_gate_active_call: gate is actually called, not just declared
  - err: `ReferenceError: pageSrcWithExternals is not defined 32 | await whPage.goto(PAGE); 33 | await waitForPageReady(whPage); > 34 | const __sentSrc_3 = await pageSrcWithExternals(whPage); | ^ 35 | const called = /await\s+isPla`
- L49 — admin_gate_not_commented: no commented-out gate redirect logic
  - err: `ReferenceError: pageSrcWithExternals is not defined 50 | await whPage.goto(PAGE); 51 | await waitForPageReady(whPage); > 52 | const __sentSrc_4 = await pageSrcWithExternals(whPage); | ^ 53 | const stripped = /\/\/[^\n]*w`
...
### `ph-intelligence.spec.ts`  (2)
- L25 — script_tag_loaded: maturity-gating script is loaded
  - err: `ReferenceError: pageSrcWithExternals is not defined 26 | await whPage.goto(PAGE); 27 | await waitForPageReady(whPage); > 28 | const __sentSrc = await pageSrcWithExternals(whPage); | ^ 29 | const has = /maturity|maturity_`
- L33 — gate_call_present: maturity gate is actually invoked
  - err: `ReferenceError: pageSrcWithExternals is not defined 34 | await whPage.goto(PAGE); 35 | await waitForPageReady(whPage); > 36 | const __sentSrc_2 = await pageSrcWithExternals(whPage); | ^ 37 | const has = /maturity/i.test(`
...
- L96 — happy path: create post — appears in feed and DB
  - err: `Error: Post "Test post from Playwright [WH-PW-53-mpjvbm04]" should appear in community_posts or feed expect(received).toBe(expected) // Object.is equality Expected: true Received: false 127 | // Accept either DB confirma`
