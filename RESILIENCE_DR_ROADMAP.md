...
## 6. C-lens DEPTH pass (2026-07-20, Ian: "another full arc" → S) — the floor was 100%, the depth found one real bug

Arc S was floor-complete, but "floor" ≠ "every write-path swept" (same as the security arc: floor-green, yet the per-surface scaffold found un-covered edge fns/views). A per-write-path idempotency audit of the **41 insert/upsert paths** (mined from `substrate/page/*.md`):
- **All 8 UPSERTs verified idempotency-safe** — 7 have a matching business-key UNIQUE index (`alert_dismissals` hive+key, `asset_nodes` hive+tag, `external_sync` sys+ext+entity, `hive_members` hive+worker, `inventory_items` part+hive, `marketplace_sellers` worker, `skill_profiles` worker); `schedule_items` upserts on a STABLE client `id` (pkey) so a re-fire updates one row.
- **★REAL FIND — `parts_staged_reservations` (asset-hub `_onStagingAccept`):** two writes fired in **parallel** via `Promise.allSettled` — the recommendation status-update was idempotency-guarded (`.is('acted_at', null)`) but the reservation **insert was not** (no UNIQUE), so a double-accept (offline-retry / timeout re-send / 2nd device racing the button-disable) made **duplicate reservations** while the status flipped once. **Client button-disable is a UI-only guard = a race window** ([[feedback_ui_only_approval_gate_is_bypassable]]). Fix: `UNIQUE(recommendation_id,item_id)` (mig `20260720000001`) + client `.upsert(onConflict, ignoreDuplicates)`; **live-proven** (duplicate rejected, upsert a no-op, rolled back clean).
- **★GATE COVERAGE GAP (the meta-find):** `validate_dedup_constraints.DEDUP_PATHS` listed only `pm_completions` + `project_links` — so the C-lens sweep was 100% GREEN while `parts_staged_reservations` sat unguarded and *unchecked*. Same class as the read-isolation `*_truth` name-filter gap: **a gate's coverage LIST silently under-covers.** Added `parts_staged_reservations` → gate now 3/3, sweep still C 100%.
- **New lesson (beyond "dedup writes need a DB UNIQUE"):** when N writes run in **parallel** and only ONE is idempotency-guarded, the others are still exposed — **guard EACH write independently; a guard on one sibling does not protect the batch.**

### 6b. The OTHER lenses' depth (F / R / D) — driven 2026-07-20 ("next all arc s depth lens")
- **R — Recovery (2 REAL findings, both FIXED):** ① **backup coverage gap** — `data_backup.py CRITICAL_TABLES` listed only **11 tables** and MISSED core user-data the RTO/RPO declaration commits to recovering: the **PM plan** (`pm_assets`, `pm_scope_items`), `hives` (tenant records), marketplace/sales (`marketplace_listings/orders/sellers`), `engineering_calcs`, `resume_documents/versions`, `community_posts/replies`, `skill_profiles`, `voice_journal_entries` (+ a **stale `assets` alias** that doesn't exist). Expanded to **23 tables**; backup now dumps **7889 rows + restore round-trip proven**. ② **robustness crash** — the tool crashed opaquely (`TypeError: data must be str, not NoneType`) on Windows: `text=True` used cp1252 while pg_dump emits UTF-8, so a clean (rc=0) dump returned a None stdout. Fixed: explicit `encoding='utf-8'` + a None-guard (a backup tool must never crash silently). *Gate-blindspot: `verify_backups` (schema reproducibility) + the sweep's R-lens both stayed GREEN while the actual data dump was broken + under-covering.*
- **F — Failure-tolerance (CLEAN):** raw-`fetch()` pages are all test/backup files (not shipped); `cdn.tailwindcss.com` survives ONLY in `*-test`/`*.backup` files (the self-host held on every real page); the 8 shipped CDN libs are guard-covered. No F-lens bug.
- **D — Degradation (scope boundary documented, not a silent gap):** `validate_offline_resilience` verifies the offline-write queue on **3 core field pages** (logbook/inventory/pm-scheduler) — the strict check (`whCreateQueue`+`startAutoSync`+`.enqueue`+`navigator.onLine`) confirms ONLY those 3 truly queue writes offline; 6 secondary field-ish pages (voice-journal, dayplanner, alert-hub, asset-hub, shift-brain, skillmatrix) *load* the shared shell but don't instantiate a write-queue → a write is lost on a brownout. Extending offline queues to them is a **feature build + product call** (are they field-offline-used?), tracked as a resilience backlog, not shipped blind.
- **★★THE CROSS-CUTTING META-LESSON (the highest-value output of this whole session):** in BOTH the security arc AND all four resilience lenses, the gates were GREEN while real gaps hid — because each gate scopes its coverage by a **hardcoded LIST or NAME pattern** that silently under-covers its siblings: read-isolation's `*_truth` name-filter (missed 3 hive_id views), `DEDUP_PATHS` (2 of the dedup-prone writes), `FIELD_WRITE_PAGES` (3 of the field pages), `CRITICAL_TABLES` (11 of the backup-critical tables). **A gate whose denominator is a curated list is only as complete as the list — audit the list against the FULL derived denominator (all `hive_id` views, all insert/upsert paths, all write-heavy pages, all user-data tables), and prefer a structural/derived denominator over a hand-list.** This is the resilience twin of the security `bughunt-scoreboard`/`edge-fn-auth-gate` "derive the denominator, don't hand-list it" discipline.
