...
(function () {
  ...
  function detectPageContext() {
    ...
    if (path.includes('logbook'))       return { page: 'logbook',       label: 'Digital Logbook',       hint: 'Help me fill in maintenance records, suggest failure modes, or explain fields. Two AI-assist surfaces: Speak to Fill (voice-logbook-entry) and Photo Defect Capture (visual-defect-capture via multimodal vision + fault_knowledge embed). The photo flow OCRs an asset tag from the image and auto-links to the canonical asset when it matches asset_nodes.tag for this hive. DOCTRINE: WorkHive is the Operational Readiness Layer for Filipino industrial plants. We never replace your ERP/CMMS, never promise predictions on insufficient data, never enforce rigid workflows, never assume enterprise infrastructure. Brownouts, 2G, shared tablets are first-class. Maturity Stairs 0-4 (Paper / Digital Logbook / Disciplined / Predictive-Ready / Industry Leader) gate advanced features. If a worker asks for predictive analytics at Stair 0-1, surface the gap honestly and link to the Maturity Stairway.' };
    ...
    if (path.includes('analytics'))     return { page: 'analytics',     label: 'Analytics Engine',       hint: 'Ask me to explain your MTBF, MTTR, Availability, or OEE results. I can interpret PM compliance scores, explain failure trends, or walk you through the AI Action Plan. Use the PDF Report button to open the print-ready version, or Send to email it to your team.' };
    ...
    if (path.includes('predictive'))        return { page: 'predictive', label: 'Predictive Maintenance', hint: 'Ask me about asset risk scores, which machines are most likely to fail, MTBF trends, or how to interpret the health heatmap. I can explain how the rules engine and ML model scores are calculated, and what each risk factor means.' };
    if (path.includes('ai-quality'))        return { page: 'ai-quality', label: 'AI Quality + ROI', hint: 'Ask me about your hive AI spend, fallback rate, schema compliance, and predicted ROI. Honest numbers: the ROI estimate carries a confidence band tied to how many thumbs ratings your workers have submitted. Below Stair 2 the page does not render — predictive numbers without baseline discipline would lie.' };
    if (path.includes('plant-connections')) return { page: 'plant-connections', label: 'Plant Connections', hint: 'Ask me about the Plant Connections Console — your CMMS integration configs, recent external sync activity, sensor topic mappings, and gateway audit. Supervisor-only surface. I can explain retention windows, sync staleness signals, and what each panel means for compliance.' };
    if (path.includes('ph-intelligence'))   return { page: 'ph-intelligence', label: 'PH Intelligence Report', hint: 'Ask me about the Philippine Industrial Maintenance benchmarks — MTBF rankings, common failure causes, seasonal patterns, or how to access the Intelligence API.' };
    if (path.includes('asset-hub'))         return { page: 'asset-hub',      label: 'Asset Hub',              hint: 'Ask me about any asset in your hive: its history, MTBF, parent equipment, sister machines, parts that fit, upcoming PMs, Reliability Workbench (FMEA + RCM + Weibull + P-F), and Live Telemetry. The Live Telemetry tile shows the last 30 days of sensor_readings per parameter with a Z-score anomaly chip (rule-based, 3-sigma threshold); data arrives from the plant MQTT/OPC-UA bridge via the sensor-readings-ingest edge function. Tap the orange Scan button next to the search bar to camera-scan an equipment QR or barcode and jump straight to that asset.' };
    if (path.includes('alert-hub'))         return { page: 'alert-hub',      label: 'Alert Hub',              hint: 'Ask me about any alert here, what it means, why it triggered, what action to take. The hub aggregates risk scores, PM overdue, low stock, pattern alerts, failed automation jobs, and the Autonomous Maintenance Crew (AMC) daily brief into one chronological feed. The AMC card at the top is generated daily at 06:00 PHT by amc-orchestrator with 5 sub-agents (Failure-Predictor, PM-Planner, Parts-Stager, Crew-Builder, Briefing-Composer); the supervisor approves the brief with one tap. Filter chips let you focus on one type at a time.' };
    if (path.includes('audit-log'))         return { page: 'audit-log',      label: 'Audit Log',              hint: 'Ask me to interpret an audit entry, find who approved or kicked someone, or export a date-range CSV for compliance review. The log records every supervisor and worker power action: approvals, rejections, joins, kicks, edits, deletes. Supervisor-only.' };
    if (path.includes('shift-brain'))       return { page: 'shift-brain',    label: 'Shift Brain',            hint: 'Ask me about the autonomous shift plan — top risk assets, PMs due today, carry-forward open work, parts to pre-stage, or how to publish to the crew. Briefings auto-generate at 06:00, 14:00, and 22:00 PHT.' };
    if (path.includes('integrations'))      return { page: 'integrations', label: 'CMMS Integration', hint: 'Ask me about importing SAP PM or Maximo data, field mapping, or what WorkHive can detect from your CMMS history.' };
...
- Shift Brain (shift-brain.html): Autonomous shift planner. At each shift change (06:00, 14:00, 22:00 PHT) the orchestrator runs four sub-agents in parallel (risk-ranking, PMs-due, carry-forward open work, parts pre-stage) plus an AI briefing synthesizer. Output is a draft hive plan; supervisor reviews and publishes to crew. Powered by pg_cron + the shift-planner-orchestrator edge function.
...
- Voice Journal (voice-journal.html): Private spoken journal. Tap the mic, talk freely in any language (English, Filipino/Tagalog, Cebuano, Ilocano, etc.); Whisper auto-detects the language, the journal companion reflects back in the same language, and entries are saved per-worker via agent_memory with rolling summary. Browser speaks replies aloud. Useful for daily reflections, lessons learned, shift wrap-ups.
...
- Alert Hub (alert-hub.html): Single chronological feed aggregating risk alerts, PM overdue, low stock, pattern alerts, and failed automation jobs. Filter chips per type. Auto-refreshes every minute. Supervisor mode tool.
- Audit Log (audit-log.html): Supervisor-only review of every recorded power action in the hive (approvals, rejections, joins, member kicks, edits, deletes). Date range chips, actor and action and target dropdowns, free-text search, expandable JSON detail per entry, CSV export for compliance. Live updates via realtime. Reads hive_audit_log written from Hive Board (approve/reject/kick) and Community (post moderation).
- Predictive Maintenance (predictive.html): Asset risk dashboard. Risk Ranking (critical/high/medium/low per asset), Health Heatmap (0-100 score), and Failure Trend forecast. Rules engine from Day 1; GBM ML activates after 500+ corrective records. Scores daily at 13:00 PHT.
- AI Quality + ROI (ai-quality.html): Supervisor dashboard showing 30-day AI spend, fallback rate, schema compliance, worker thumbs feedback, predicted ROI with confidence band, and per-function spend drill-down. Stair 2+ gated; below that the page renders an honest empty state instead of fabricated ROI numbers.
- Plant Connections (plant-connections.html): Supervisor-only operations surface unifying CMMS integration configs, last-30-day external sync activity, sensor topic mappings, gateway audit (last 7d), data retention windows, and SSO config. Phase 5 Track C — single place to triage integration staleness and review compliance posture.
- PH Intelligence (ph-intelligence.html): Philippine Industrial Maintenance Intelligence Report — monthly anonymized benchmarks across WorkHive plants: MTBF rankings by equipment category, top failure causes, seasonal insights, plus WorkHive Intelligence API access for third-party integrations.
- CMMS Integration (integrations.html): Import SAP PM, IBM Maximo, or any CSV/Excel CMMS export. 5-step wizard: choose source, upload file, auto-detect columns, map fields, preview, and batch import to external_sync and logbook. Activates Tribal Knowledge Surface, Failure Signature Detection, and AI Reliability Coach immediately from historical data.
