...
- Digital Logbook (logbook.html): Log daily maintenance jobs — machine, problem, root cause, action taken, downtime, parts used, failure consequence (Hidden/Running reduced/Safety risk/Stopped production), sensor readings (temperature, vibration, pressure), and production output (good units / total units for OEE quality). When status = Closed, a Parts Used section appears. Supports asset linking and photo uploads.
...
- Community Board (community.html): Hive-scoped discussion board. Post threads (General, Safety, Technical, Announcements), reply, react with emojis. Supervisors pin, flag, and moderate content. Leaderboard shows top XP contributors. Live presence.
...
- Report Sender (report-sender.html): PWA tool for generating and emailing maintenance reports. Select report types (PM Overdue, Failure Digest, Shift Handover, Predictive), save contacts, use voice context to focus reports on specific machines, and send to multiple recipients. Installable as a home screen app.
- Analytics Engine (analytics.html): Stage 3 maintenance intelligence dashboard. 4 phases: Descriptive (MTBF, MTTR, Availability, OEE — using production output from Logbook, PM Compliance, Failure Frequency, Downtime Pareto), Diagnostic (RCM Consequence Analysis using failure_consequence field, PM-Failure Correlation, Skill-MTTR Correlation), Predictive (Next Failure Date, PM Due Calendar, Sensor Anomaly Detection using readings_json, Parts Stockout, Health Score), Prescriptive (Priority Ranking, PM Interval Optimization, Tech Assignment, WorkHive AI Action Plan). Standards: ISO 14224, ISO 13381-1, ISO 55000, SAE JA1011, SMRP, ISO 22400-2 (OEE).
- Community Board (community.html): Hive-scoped discussion board. Post threads by category (General, Safety, Technical, Announcements), reply, and react with emojis. Supervisors pin posts, flag content, and manage the mod queue. Leaderboard shows top XP contributors. Live presence shows who is online.
