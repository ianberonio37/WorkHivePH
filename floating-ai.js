...
(function () {
  ...
  function detectPageContext() {
    ...
    if (path.includes('asset-hub'))         return { page: 'asset-hub',      label: 'Asset Hub',              hint: 'Ask me about any asset in your hive — its history, MTBF, parent equipment, sister machines, parts that fit, or upcoming PMs. The hub is the unified 360 view across logbook, PM, inventory, predictive, and PH benchmarks. Tap the orange Scan button next to the search bar to camera-scan an equipment QR or barcode and jump straight to that asset.' };
    if (path.includes('alert-hub'))         return { page: 'alert-hub',      label: 'Alert Hub',              hint: 'Ask me about any alert here — what it means, why it triggered, what action to take. The hub aggregates risk scores, PM overdue, low stock, pattern alerts, and failed automation jobs into one chronological feed. Filter chips let you focus on one type at a time.' };
