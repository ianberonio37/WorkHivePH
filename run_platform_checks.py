...
VALIDATORS = [
    ...
    },
    {
        ...
        "args":    [],
        "label":   "Per-page P3 CRUD-at-DB gate for SIDE-EFFECT tables (LIVE, rolled-back psql) — `logbook` / `inventory_items` / `resume_documents` / `report_contacts` can't go through `page-crud` because a persisted INSERT fires expensive/stateful triggers (embed http_request, achievement XP, rate-limit, daily-cap). Instead runs a single WORKER-JWT transaction per table (INSERT with FORGED worker_name -> assert PINNED by bind_*_submitter (where applicable; personal resume/hive-shared contacts have no pin) + correct hive_id/auth_uid -> own-scoped UPDATE -> own-scoped DELETE) then ROLLS BACK (undoes the row AND the after-commit side-effects = 0 pollution). Data-driven (each table emits its own pin/hive/auth booleans in SQL). Bug-hunt roadmap P3, 2026-07-19. A dropped bind trigger / broken own-write RLS / lost hive_id FAILs. Skips if docker/DB unreachable. Self-test: --selftest.",
        "group":   "Platform",
        "skip_if_fast": True,
        "severity": "fail",
    },
