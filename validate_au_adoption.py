"""
API-Adoption Ratchet (L0) — FULLSTACK_COMPONENT_LIBRARY_ROADMAP Layer AU, AU-P2.
================================================================================
...
au_component_registry.json row, recomputed LIVE over every edge function's
index.ts (never a stale report):

  A1  ADOPTION FLOOR (FAIL) — live adopter count >= the floor in
      au_adoption_baseline.json for every measured row. A function dropping
      its session/identity floor call is a regression.
...
Output: au_adoption_gate_report.json (+ rewrites au_adoption_baseline.json on
...
from au_adoption_census import run_census  # noqa: E402

BASELINE = ROOT / "au_adoption_baseline.json"
REPORT = ROOT / "au_adoption_gate_report.json"
...
def write_baseline(rows, n_fns):
    ...
    BASELINE.write_text(json.dumps({
        "_doc": "Layer AU adoption baseline. Floors are FORWARD-ONLY from any writer (Layer F law).",
...
def main() -> int:
    ...
    if not BASELINE.exists():
        ...
        print(f"au-adoption: first run — baseline created ({len(measured)} floors). PASS")
        return 0
    ...
    print(f"au-adoption: {len(measured)} measured rows"
