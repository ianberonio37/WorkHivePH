...
**Definition (collapsed):**  SELECT pa.hive_id, pa.id AS pm_asset_id, pa.asset_name, pa.tag_id, pa.category, pa.criticality, pa.location, pa.last_anchor_date, CASE WHEN (max(pc.completed_at) FILTER (WHERE (pc.status = 'done'::text)) IS NULL) THEN NULL::integer ELSE ((now())::date - (max(pc.completed_at) FILTER (WHERE (pc.status = 'done'::text)))::date) END AS days_since_last_completion, count(pc.id) FILTER (WHERE (pc.status = 'done'::text)) AS lifetime_completions, count(pc.id) FILTER (WHERE ((pc.status = 'done'::text) AND (pc.completed_at >= (now() - '30 days'::interval)))) AS completions_30d, count(pc.id) FILTER (WHERE …
