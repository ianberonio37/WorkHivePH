...
**Definition (collapsed):**  SELECT DISTINCT ON (hive_id, asset_id, COALESCE((fmea_mode_id)::text, '_'::text)) id AS fit_id, hive_id, asset_id, fmea_mode_id, beta, eta_days, failure_pattern, n_failures, n_censored, fit_method, log_likelihood, source_window_days, diagnostic, generated_at FROM weibull_fits ORDER BY hive_id, asset_id, COALESCE((fmea_mode_id)::text, '_'::text), generated_at DESC;
