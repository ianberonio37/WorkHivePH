...
**Definition (collapsed):**  SELECT id, segment, category, name, description, unit, base_rate, active, created_at, updated_at, 1 AS _source_count, updated_at AS _freshness_ts, 'service_catalog_truth:v1'::text AS _canonical_version FROM service_catalog c WHERE (active = true);
