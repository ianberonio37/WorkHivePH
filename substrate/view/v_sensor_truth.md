...
**Definition (collapsed):**  SELECT DISTINCT ON (hive_id, asset_id, parameter) id AS reading_id, hive_id, asset_id, parameter, value, unit, quality_flag, recorded_at, source, is_anomaly FROM sensor_readings s ORDER BY hive_id, asset_id, parameter, recorded_at DESC;
