...
**Definition (collapsed):**  SELECT wp.auth_uid, wp.username, wp.display_name AS worker_name, wp.email, wp.preferred_persona, wp.created_at AS registered_at, hm.hive_id, hm.role, hm.joined_at AS hive_joined_at, hm.status AS hive_status, (hm.hive_id IS NULL) AS is_solo, ( SELECT count(*) AS count FROM hive_members hm2 WHERE ((hm2.auth_uid = wp.auth_uid) AND (hm2.status = 'active'::text))) AS active_hive_count FROM (worker_profiles wp LEFT JOIN hive_members hm ON (((hm.auth_uid = wp.auth_uid) AND (hm.status = 'active'::text))));
